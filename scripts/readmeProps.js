...
const replaceBetween = (str, start, end, replace) => {
  ...
  if (startIdx !== -1 && endIdx !== -1) {
    const between = str.substring(startIdx + start.length + 1, endIdx - 1);
...
const extractTypesInfo = (typeObject, name) => {
  switch (typeObject.type) {
    ...
    case "reference":
      const allowedPackages = ["@types/react", "@types/react-dom", "@ammar-ahmed22/react-chess", "typescript"];
      const typeParams = typeObject.typeArguments && typeObject.typeArguments.map(t => extractTypesInfo(t));
      if (allowedPackages.includes(typeObject.package)) {
        if (typeParams) return `${typeObject.name}&lt;${typeParams.join(",")}&gt;`
        return typeObject.name;
      }
    ...
    case "union":
      if (Array.isArray(typeObject.types)) {
        return typeObject.types.map(t => extractTypesInfo(t)).join(" &#124; ");
      };
      return "union"
    case "reflection":
...
const extractDocsInfo = (props) => {
  ...
  for (let prop of props) {
    const info = {}
    info.name = prop.name;
    info.description = prop.comment.summary.map(o => o.text.replace("\n", "<br />")).join("");
    info.default = "N/A";
    info.params = "";
    if (prop.comment.blockTags) {
      for (let tag of prop.comment.blockTags) {
        if (tag.tag === "@default") {
          info.default = `\`${tag.content.map(o => o.text).join("").split("\n")[1]}\``;
        }
        if (tag.tag === "@param") {
          if (info.params === "") {
...
async function main() {
  ...
  if (project) {
    ...
    const updated = replaceBetween(readme, "<!-- CHESSBOARD PROPS TABLE START -->", "<!-- CHESSBOARD PROPS TABLE END -->", genTable);
