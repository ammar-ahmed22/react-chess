...
import { Chess, HalfMove, SquareID } from "@ammar-ahmed22/chess-engine";
// import { SquareType } from "../../hooks";
import React, { useState, useEffect } from "react";


type UsedChessBoardProps = "position" | "flipBoard" | "validMoves" | "onSquareClick" | "squareDraggable" | "onSquareDragStart" | "onSquareDrop" | "onSquareDragOver" | "onSquareDrag" | "onSquareDragEnd" | "onSquareDragEnter" | "onSquareDragLeave";
const Game: React.FC<Omit<ChessBoardProps, UsedChessBoardProps>> = (props) => {
  ...
  useEffect(() => {
    if (fromID && toID) {
      const [executeMove] = chess.validMoves().filter(move => {
        ...
        return (from.algebraic === fromID.algebraic && to.algebraic === toID.algebraic);
      })
      ...
      <ChessBoard 
        ...
        onSquareClick={(square) => {
          for (let move of validMoves) {
            ...
            if (fromID?.algebraic === from.algebraic && to.algebraic === square.algebraic) {
              setToID(to);
            }
          }
        }}
        ...
        onSquareDragOver={(square) => {
          ...
          for (let move of validMoves) {
            const to = SquareID.fromSquareIDType(move.to);
            const from = SquareID.fromSquareIDType(move.from);
            if (fromID.algebraic === from.algebraic && to.algebraic === square.algebraic) return true;
          }
          return false;
        }}
